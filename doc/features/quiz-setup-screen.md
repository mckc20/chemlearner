...
An intermediate configuration screen between selecting compounds and starting a quiz. Users choose the quiz type and number of questions before starting.
...
|---|---|---|---|
| `formula-from-name` | Formula from Name | Compound name (text) | 4 formula choices (text) |
...
| `structure-from-name` | Structure from Name | Compound name (text) | 4 small 3D viewers (2x2 grid) |
...
- General Knowledge type is disabled when no GK questions exist for selected compounds
...
- **QuizMode.jsx**: `buildQuiz` branches on all 6 quiz types; accepts `quizConfig` and `getQuestionsForCompounds` props
